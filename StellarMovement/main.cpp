...
void render_loop(int& current_frame) {
    ...
    //    std::cout << "Prev Pos: " << trajectory.col(current_frame-1) <<
    //    std::endl; std::cout << "Earth position: " << std::endl<<
    //    trajectory.col(current_frame) << std::endl; std::cout << "Earth
    //    velocity: " << std::endl<< Earth.velocity << std::endl;
    //  }
...
int main(int argc, char* argv[]) {
    ...
    glOrtho(-800., 800., -800., 800., 0.f,
            1.f); // this creates a canvas you can do 2D drawing on

    Earth.position = Eigen::Vector3f(AU, 0, 0);      // 1 AU right of the Sun
    Earth.velocity = Eigen::Vector3f(0, 29780.f, 0); // 29.78 km/s in y
                                                     // direction
